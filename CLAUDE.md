...
- **League re-sync 500'd on `uq_roster_player` — `clear()` + re-add on an orphanRemoval collection races its own flush**: `FantasyLeagueService.syncLeague()` used to do `team.getRosterPlayers().clear()` then build brand-new `FantasyRosterPlayer` objects for every currently-rostered player and add them back, relying on `FantasyTeam.rosterPlayers`' `orphanRemoval = true` to delete the old rows. Hibernate's flush action ordering runs collection **inserts before deletes** in the same flush, so for any player who was on the roster both before and after (the normal case — most of a 16-man roster doesn't change week to week), the INSERT of their "new" row hit the DB before the DELETE of their old row had flushed, violating the `(fantasy_team_id, sleeper_player_id)` unique constraint (`uq_roster_player`) and throwing `DataIntegrityViolationException` for the whole `@Transactional` sync — a 500 on every re-sync of a league whose roster wasn't 100% turned over. Also meant the manual "Re-sync" button (`LeagueController.sync()`) looked broken with zero feedback, since that handler didn't surface the exception either (see the flash-message fix alongside this one). Fixed by diffing instead of clear-and-recreate: build a `sleeperPlayerId -> existing FantasyRosterPlayer` map first, `removeIf` only the ids no longer on the roster (still relies on orphanRemoval, but now only for genuine drops), then for each currently-rostered id either update the existing row's `starter` flag in place or add a brand-new row only if it's truly new. Unchanged players never get a delete+insert pair for the same natural key, so the constraint can't fire. Any future collection resync of a similarly-keyed `@OneToMany` should diff, not clear-and-recreate.
